...
```{r create_project, include=FALSE}
...
```{r ade4-pcoa}
trait_pcoa = ade4::dudi.pco(ade4::quasieuclid(gower_dissim), nf = 5,
                            scannf = FALSE)
trait_pcoa
```
...
* **Q15**: How does the relationship between indices with species richness compare with total biomass values? (You can use the function `cor.test()` if you want to test the association)
...
null_fd = lapply(null_traits, function(x) {
  
  # Specific code to avoid edge cases when computing null FRic
  null_fric = lapply(rownames(sp_com), function(y) {
    
    tryCatch({
      fundiversity::fd_fric(x, sp_com[y,, drop = FALSE], stand = FALSE)
    },
    error = function(error) {
      data.frame(site = y, FRic = NA)
    })
    
  })
  # Combine all null FRic values
  null_fric_df = do.call(rbind.data.frame, null_fric)
